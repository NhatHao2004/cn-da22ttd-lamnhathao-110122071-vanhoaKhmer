/**
 * Quiz Admin JavaScript - Chùa Khmer
 */
...
async function viewQuestions(quizId, quizTitle) {
    ...
    try {
        const response = await fetch(`quiz-chua-questions-api.php?quiz_id=${quizId}`);
...
function editQuestion(question, answers) {
    ...
    answers.forEach((answer, index) => {
        ...
        if (maDapAnInput) maDapAnInput.value = answer.ma_dap_an;
        if (dapAnInput) dapAnInput.value = answer.noi_dung;
