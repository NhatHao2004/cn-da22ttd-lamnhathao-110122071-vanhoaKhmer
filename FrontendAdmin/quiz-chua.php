...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch($action) {
            case 'create_quiz':
                $maChua = intval($_POST['ma_chua']);
                $tieuDe = trim($_POST['tieu_de']);
                $moTa = trim($_POST['mo_ta'] ?? '');
                $thoiGian = intval($_POST['thoi_gian'] ?? 600);
                
                $sql = "INSERT INTO quiz (loai_quiz, ma_doi_tuong, tieu_de, mo_ta, thoi_gian) VALUES ('chua', ?, ?, ?, ?)";
                $db->execute($sql, [$maChua, $tieuDe, $moTa, $thoiGian]);
            ...
            case 'delete_quiz':
                ...
                $db->execute("DELETE FROM quiz WHERE ma_quiz = ? AND loai_quiz = 'chua'", [$maQuiz]);
            ...
            case 'toggle_status':
                ...
                $sql = "UPDATE quiz SET trang_thai = ? WHERE ma_quiz = ? AND loai_quiz = 'chua'";
            ...
            case 'edit_quiz':
                ...
                $sql = "UPDATE quiz SET tieu_de = ?, mo_ta = ?, thoi_gian = ? WHERE ma_quiz = ? AND loai_quiz = 'chua'";
...
$quizzes = $db->query("
    ...
    FROM quiz q
    JOIN chua_khmer c ON q.ma_doi_tuong = c.ma_chua
    WHERE q.loai_quiz = 'chua'
    ORDER BY q.ngay_tao DESC
");

// Đảm bảo $quizzes là array
if (!is_array($quizzes)) {
    $quizzes = [];
}

// Lấy danh sách chùa
$chuaList = $db->query("SELECT ma_chua, ten_chua FROM chua_khmer ORDER BY ten_chua");
if (!is_array($chuaList)) {
    $chuaList = [];
}
...
<head>
    ...
    <title>Quản Lý Quiz Chùa Khmer</title>
...
<body>
        ...
        <div class="page-header">
            <div class="page-header-content">
                <div class="page-title-wrapper">
                    <div class="page-icon-wrapper" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                        <i class="fas fa-place-of-worship" style="font-size: 2rem;"></i>
                    </div>
                    <div>
                        <h1>Quản Lý Quiz Chùa Khmer</h1>
                    </div>
                </div>
                <div style="display: flex; gap: 1rem; flex-wrap: nowrap;">
                    <a href="chua.php" class="btn-add-new btn-back">
                ...
                <table class="modern-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= htmlspecialchars($quiz['ten_chua']) ?></td>
                            <td>
    ...
    <!-- Modals -->
    <?php include 'includes/quiz-modals-chua.php'; ?>
    
    <!-- Scripts -->
    <script src="assets/js/quiz-admin-chua.js"></script>
</body>
</html>
